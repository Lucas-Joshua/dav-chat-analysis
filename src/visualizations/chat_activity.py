"""Chat activity visualizations and registry entries."""
...
def plot_chat_activity_weekday_weekend(
    ...
    if "date_only" not in df.columns or "is_weekend" not in df.columns:
        raise KeyError("date_only and is_weekend columns not found.")

    out_path = ensure_parent_dir(out_path)
    working = df.dropna(subset=["date_only", "is_weekend"]).copy()
    ...
    color_weekday = DEFAULT_PLOT_SETTINGS.neutral_color
    color_weekend = DEFAULT_PLOT_SETTINGS.danger_color
    ...
    half_width = 0.32
    ax.hlines(median_weekday, 1 - half_width, 1 + half_width, colors=DEFAULT_PLOT_SETTINGS.text_color, linewidths=2.0, linestyles="solid", zorder=5)
    ax.hlines(median_weekend, 2 - half_width, 2 + half_width, colors=DEFAULT_PLOT_SETTINGS.text_color, linewidths=2.0, linestyles="solid", zorder=5)

    for x_pos, values, color in [(1, weekday, color_weekday), (2, weekend, color_weekend)]:
        q25 = float(np.percentile(values, 25))
        q75 = float(np.percentile(values, 75))
        ax.fill_betweenx([q25, q75], x_pos - 0.26, x_pos + 0.26, color=color, alpha=0.14, zorder=1)
...
def chat_activity_by_hour(df, out_dir: str | Path | None = None) -> None:
    """Generate the chat activity by-hour chart."""
